...
int main (){
	ifstream intputFileIdentifier;
	ofstream outputFileIdentifier;
	intputFileIdentifier.open("filename.txt");
	outputFileIdentifier.open("filename.txt");
	srand(time(NULL));
	...
	system("pause");
	intputFileIdentifier.close();
	outputFileIdentifier.close();
	return 0;
}